...
root directory of gluu chroot jail is `/opt/gluu-server-3.1.8/` 

download `/opt/gluu-server-3.1.8/root/configs.tgz` to your machine. 
...
3\. Copy into custom Gluu schema folder 2 files from CE `/opt/gluu-server-3.1.8/opt/gluu/schema/openldap`
...
```
 ...
 - Copy from CE file /opt/gluu-server-3.1.8/opt/symas/etc/openldap/slapd.conf sections into `C:\Program Files (x86)\symas-openldap\etc\openldap\slapd.conf`:
```
   #######################################################################
