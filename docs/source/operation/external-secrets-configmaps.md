## Overview
   This guide shows how to store and retrieve gluu  `configmaps` and `secrets` externally in `AWS Secrets Manager` and `GCP Secret Manager`.
...
`Configmaps` are stored in a single `String Secret`. It follows the naming convention of `gluu_configs`

`Secrets` are splitted and stored in multiple `Binary Secrets` due to the max size limitation.

 - `Secrets` follows the naming convention of `gluu_secrets`, `gluu_secrets_1`, `gluu_secrets_2`..etc
 ...
 - Every single secret doesn't have to be a valid json, but instead the collection of all secrets should have a valid json. For example: 
      ...
      `gluu_secrets`: {"key1":"value1",

      `gluu_secrets_1`: "key2":"value2",
      
      `gluu_secrets_2`: "key3":"value3"}
...
config:
  configmap:
    ...
    cnAwsProfile: gluu #Choose based on your aws named profile
...
- Get the json of the configmap
  ```bash
  kubectl get configmap -n <namespace> gluu -o json 
  ```
...
- Get the json of the secret
  ```bash
  kubectl get secret -n <namespace> gluu -o json 
  ```
