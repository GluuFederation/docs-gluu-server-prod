...
```
$ tail -f /opt/gluu-server-3.1.8/opt/gluu/jetty/identity/logs/* \
          /opt/gluu-server-3.1.8/opt/gluu/jetty/oxauth/logs/*
```       
...
```
# /etc/init.d/gluu-server-3.1.8 stop
# rm -rf /opt/gluu-server-3.1.8/opt/jetty-9.x/temp/*
# /etc/init.d/gluu-server-3.1.8 start
```
...
1. Download the script:     
          ```
          # wget https://raw.githubusercontent.com/GluuFederation/community-edition-setup/version_3.1.8/post-setup-add-components.py
          ```
...
```
# /etc/init.d/gluu-server-3.1.8 login
...
You can find the configuration you need in `/opt/gluu-server-3.1.8/etc/gluu/conf/ox-ldap.properties`, e.g.:
