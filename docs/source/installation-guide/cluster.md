...
159.203.1.8.10  cluster.example.org (Gluu Server 3.1.8 on Ubuntu 16.04 ) (Node -1 )
138.197.65.243  cluster.example.org (Gluu Server 3.1.8 on Ubuntu 16.04 ) (Node -2 )
...
- Gluu Server 3.1.8 using OpenDJ 
...
```bash
scp /opt/gluu-server-3.1.8/install/community-edition-setup/setup.properties.last myuser@138.197.65.243:/opt/gluu-server-3.1.8/install/community-edition-setup/setup.properties
```
...
```bash
chown <user> /opt/gluu-server-3.1.8/install/community-edition-setup/
```
...
```bash
chown root /opt/gluu-server-3.1.8/install/community-edition-setup/
```

- If the Gluu server has not been started, start it and log in. Once the `setup.properties` file is in place on the **other** node(s), modify the IP to the current node. In the example, there is only one, so we changed our `ip=159.203.1.8.10` of node 1 to the IP of node 2 server which is `ip=138.197.65.243.
...
```bash
scp /opt/gluu-server-3.1.8/opt/opendj/config/opendj_crts.tar  myuser@138.197.65.243:/opt/gluu-server-3.1.8/opt/opendj/config/
```
...
```bash
/opt/opendj/bin/dsreplication status -n -X -h 159.203.1.8.10  -p 4444 -I admin -w <password>
```
...
key /etc/csync2.key;
include /opt/gluu-server-3.1.8/opt/gluu/jetty/identity/conf/shibboleth3/idp/;
include /opt/gluu-server-3.1.8/opt/gluu/jetty/identity/conf/shibboleth3/sp/;
include /opt/gluu-server-3.1.8/opt/shibboleth-idp/conf;
include /opt/gluu-server-3.1.8/opt/shibboleth-idp/metadata/;
include /opt/gluu-server-3.1.8/opt/shibboleth-idp/sp/;
include /opt/gluu-server-3.1.8/opt/shibboleth-idp/temp_metadata/;
include /opt/gluu-server-3.1.8/etc/gluu/conf/;
...
```bash
scp /opt/gluu-server-3.1.8/etc/certs/httpd.key user@loadbalancer.example.org:/etc/nginx/ssl/
scp /opt/gluu-server-3.1.8/etc/certs/httpd.crt user@loadbalancer.example.org:/etc/nginx/ssl/
```
...
```
scp /opt/gluu-server-3.1.8/etc/certs/* root@138.197.65.243:/opt/gluu-server-3.1.8/etc/certs/
```
...
Gluu.Root # logout
service gluu-server-3.1.8 restart
```
