...
### New Support for RDBMS

The Gluu Server now supports additional backend options: MySQL, Spanner, and Aurora. Migration from an existing backend to a new one is not currently possible, but a migration process will be implemented in an upcoming patch.

### New Interception Scripts

As with most releases, 4.3 comes packaged with a number of new and updated interception scripts. The new version includes scripts to enable Apple TouchID, PingID, and passwordless authentication out of the box.

### Impossible Travel

Gluu now integrates with Deduce to provide "impossible travel" protection. For example, if a user tries to log in from San Francisco, then again from Beirut thirty minutes later, and it's impossible to travel that distance in that time, it's unlikely to be a legitimate login request and may require further authentication steps.

### Updated SAML Integration

Since the version of Shibboleth packaged with Gluu has been updated, some new SAML features are supported. 
