...
### New Support for RDBMS in Cloud Native Distributions

The Gluu Server Cloud Native distribution now supports additional backend options: MySQL, Spanner, and Aurora. Migration from an existing backend to a new one is not currently possible, but a migration process will be implemented in an upcoming patch.

### SCIM API Improvements

More contextual information, better result filtering, and two new security options are available (OAuth and disabled). 

### More Metrics

In 4.3, we've enabled tracking of Monthly Active Users, which is valuable information when planning scaling.

### VM Distribution Changes

Introduced in version 4.3, we now have VM packages for SUSE Enterprise Linux and a no-chroot package built in RHEL 8 with the DISA STIG security profile. Additionally, we've ended support for the Debian distribution.

### Shibboleth IDP Upgraded to Version 4.1

### New Interception Scripts

As with most releases, 4.3 comes packaged with a number of new and updated interception scripts. The new version includes scripts to support FIDO Platform Authenticators including Apple TouchID, PingID, impossible travel, and passwordless authentication out of the box.
