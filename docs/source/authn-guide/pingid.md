...
![flow](https://github.com/GluuFederation/oxAuth/raw/version_4.3.0/Server/integrations/pingid/flow.png)
...
=== "Community Edition - VM"
    ...
    Extract [this file](https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/bundle.zip) to the root (ie. `/`) of your Gluu server. In a standard CE installation this means extraction should take place under `/opt/gluu-server`.
    ...
    1. Transfer this [file](https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/oxauth-pingid-1.0.jar) to your Gluu instance inside folder `/opt/gluu/jetty/oxauth/custom/libs`. 
...
=== "Cloud Native Edition - Kubernetes"
    ...
    -   Extract script assets from [this file](https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/bundle.zip).
    
        ```bash
        wget https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/bundle.zip && unzip bundle.zip -d bundle
        ```
    
    - Get oxauth-pingid library from this [file](https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/oxauth-pingid-1.0.jar). If you want to generate the jar file on your own (and possibly inspect the source code) follow the steps [below](#building-oxauth-pingid-library)
    
        ```bash
        wget https://github.com/GluuFederation/oxAuth/raw/version_4.3.2/Server/integrations/pingid/oxauth-pingid-1.0.jar
        ```
    ...
    === "Jackrabbit"
        ...
        1. After connecting to  Jackrabbit move the extracted content from bundle.zip  and the oxauth-pingid library which should be mapped correctly as below inside jackrabbit
...
1.  Visit `Configuration` > `Person Authentication Scripts`. At the bottom click on `Add custom script configuration` and fill values as follows:
    ...
    - In the `script` field use the contents of this [file](https://github.com/GluuFederation/oxAuth/raw/version_4.3.0/Server/integrations/pingid/pingIDAuthenticator.py)
...
1.  In a development machine, ensure Java 8+ and Maven build tool are installed. [Download](https://github.com/GluuFederation/oxAuth/archive/refs/heads/version_4.3.2.zip) oxAuth project  and extract it to a temporary location
